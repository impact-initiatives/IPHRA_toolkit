...
main <- data.list$main %>% 
  ...
  addindicators::add_hhs(hhs_nofoodhh_1 = "hhs_lack_food",
                         ...
                         yes_answer = "oui", no_answer = "non", rarely_answer = "rarement", 
  sometimes_answer = "parfois", often_answer = "souvent") %>%
...
# ### FOR MSNA CAR
num_children <- data.list$hh_roster %>% 
  rename(uuid = "uuid6") %>% 
  group_by(uuid) %>% 
  summarise(nombre_garcons_0_2 = sum(as.numeric(garcons_0_2),na.rm = T),
            nombre_filles_0_2 = sum(as.numeric(filles_0_2),na.rm = T),
            nombre_garcons_3_5 = sum(as.numeric(garcons_3_5),na.rm = T),
            nombre_filles_3_5 = sum(as.numeric(filles_3_5),na.rm = T))

main <- main %>%
  left_join(num_children, by ="uuid") %>% 
  mutate(num_children = rowSums(across(c(nombre_garcons_0_2,nombre_filles_0_2,nombre_garcons_3_5,nombre_filles_3_5), .fns = as.numeric), na.rm = T))
...
write_xlsx(sheets, "output/quality_report/quality_plausibility_report_car_msna.xlsx")
```

```{r}
library(healthyr)
(plot_ridge_distribution(flag, numeric_cols = c("fcs_cereal", "fcs_dairy", "fcs_veg", "fcs_fruit", "fcs_legumes", "fcs_sugar", "fcs_oil"),
                         name_groups = "Food Groups", name_units = "Days"))

(plot_ridge_distribution(flag, numeric_cols = c("fcs_cereal", "fcs_dairy", "fcs_veg", "fcs_fruit", "fcs_legumes", "fcs_sugar", "fcs_oil"),
                         name_groups = "Food Groups", name_units = "Days", grouping = "enumerator"))

(plot_ridge_distribution(flag, numeric_cols = c("rcsi_lessquality", "rcsi_borrow", "rcsi_mealsize","rcsi_mealadult", "rcsi_mealnb"),
                         name_groups = "Food Coping Strategy", name_units = "Days"))

(plot_ridge_distribution(flag, numeric_cols = c("rcsi_lessquality", "rcsi_borrow", "rcsi_mealsize","rcsi_mealadult", "rcsi_mealnb"),
                         name_groups = "Food Coping Strategy", name_units = "Days", grouping = "enumerator"))

(plot_correlogram(flag, numeric_cols = c("fcs_score",  "rcsi_score",  "hhs_score")))
...
run_fsl_monitoring_dashboard_test(df = flag, grouping_var = "enumerator", filter_var1 = "cluster")
```
