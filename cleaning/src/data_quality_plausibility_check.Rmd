...
```{r}
(plot_agepyramid_phu(table_age_pyramide))
```

#### Age Years Distibution (0 - 10)

```{r}
(plot_age_years_distribution_phu(table_age_pyramide %>% rename(age_years = "ind_age_years") %>% 
...
if(all(fcs_check_columns %in% names(raw.flag.fcs))) {
  ...
  (plot_ridge_distribution_phu(raw.flag.fcs, numeric_cols = c("fcs_cereal", "fcs_dairy", "fcs_veg", "fcs_fruit", "fcs_legumes", "fcs_sugar", "fcs_oil"),
...
if(all(rcsi_check_columns %in% names(raw.flag.fcs))) {
  ...
  (plot_ridge_distribution_phu(raw.flag.fcs, numeric_cols = c("rcsi_lessquality", "rcsi_borrow", "rcsi_mealsize","rcsi_mealadult", "rcsi_mealnb"),
...
if(all(fcs_check_columns %in% names(raw.flag.fcs)) & 
  ...
  (plot_correlogram_phu(raw.flag.fcs, numeric_cols = c("fcs_score",  "rcsi_score",  "hhs_score")))
}
```
...
(plot_age_months_distribution_phu(raw.flag.nut, age_min = 6, age_max = 59))
```

#### Age Month Distribution per Enumerator

```{r}
(plot_age_months_distribution_phu(raw.flag.nut, age_min = 6, age_max = 59,by_group = "enumerator"))
```
...
```{r}
(plot_cumulative_distribution_phu(df = raw.flag.nut, index = "muac", flags = "no"))
```
...
```{r}
(plot_cumulative_distribution_phu(df = raw.flag.nut, index = "muac", flags = "yes", grouping = "enumerator"))
```
