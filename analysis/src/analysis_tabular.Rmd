...
if(exists("results.summary")){
  plot_results.summary <- ggplot2::ggplot(results.summary %>% 
                                               mutate(variable = case_when(variable == "cmr"~"Crude",
                                                                           variable == "u5mr"~"Under 5")), aes(x = variable)) + geom_hline(yintercept=1, linetype='dotted', color = 'black', size =1)+
    geom_hline(yintercept=2, linetype='dotted', color = 'red', size =1)+
    geom_errorbar(aes(ymin =`95%lci`, ymax = `95%uci`, width = 0.2, color = "red"))+
    geom_text(aes(x = variable, y = `95%lci`, label = `95%lci`, vjust = 1.5, color = "red", fontface = "bold"), size = 3)+
    geom_text(aes(x = variable, y = `95%uci`, label = `95%uci`, vjust = -0.5, color = "red", fontface = "bold"), size = 3)+
  
    geom_point(aes(y = point.est), color = "blue")+
    geom_text(aes(x = variable, y = point.est, label = point.est, hjust = -0.5), size = 3, color = "blue", fontface = "bold")+
    scale_y_continuous(limits=c(0, under5$x_upp))+
    theme(legend.position = "none",
          axis.title.y = element_blank())+
    xlab("Overall HH Survey")+ 
    labs(caption = c("Black Dotted Line: Crude Threshold (WHO)", "Red Dotted Line: Under 5 Threshold (WHO)")) + 
    theme(plot.caption = element_text(hjust = c(0.2,0.8),color = c("black","red")))
}
```
...
if(exists("results.summary_bySex")){
  plot_results.summary_bySex <- ggplot2::ggplot(results.summary_bySex %>% 
    ...
    geom_point(aes(y = point.est), color = "blue")+
    geom_text(aes(x = variable, y = point.est, label = point.est, hjust = -0.5), size = 3, color = "blue", fontface = "bold")+
    scale_y_continuous(limits=c(0, max(results.summary_bySex$`95%uci`)))+
    theme(legend.position = "none",
          axis.title.y = element_blank())+
    xlab("Overall HH Survey")+ 
    facet_wrap(~Sex)+
...
if(as.numeric(strings['population_estimation']) > 10000){
  nut_survey <- as_survey_design(nut)
} else {
  nut_survey <- as_survey_design(nut,
                          fpc = fpc)
}
...
```{r, include=FALSE}
nut_assessment <- nut %>% 
  mutate(oedema_less_2 = ifelse(edema_confirm == "yes"& age_months < 24,1,0),
         muac_115_less_2 = ifelse(muac<11.5 & age_months < 24,1,0),
         muac_115_124_less_2 = ifelse(muac>=11.5&muac<=12.4& age_months < 24,1,0),
         muac_125_less_2 = ifelse(muac>12.4& age_months < 24,1,0),
         oedema_more_2 = ifelse(edema_confirm == "yes"& age_months >= 24,1,0),
         muac_115_more_2 = ifelse(muac<11.5 & age_months >= 24,1,0),
         muac_115_124_more_2 = ifelse(muac>=11.5&muac<=12.4& age_months >= 24,1,0),
         muac_125_more_2 = ifelse(muac>12.4& age_months >= 24,1,0),
         sex = case_when(sex == 1 ~ "Male",
                         sex == 2 ~ "Female")) %>% 
  group_by(sex) %>% 
  summarise(
