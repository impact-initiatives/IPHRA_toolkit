...
```{r}
cat(paste0("<h5>Number of detected inconsistency between main and water_count_loop:<strong> ",if(exists('loop_counts_main_3'))
{nrow(loop_counts_main_3)
}else {
0
},"</strong></h5>"))
```
...
```{r}
cat(paste0("<h5>Number of detected inconsistency between main and women:<strong> ",if(exists('loop_counts_main_5'))
{nrow(loop_counts_main_5)
}else {
0
},"</strong></h5>"))
```
...
```{r}
cat(paste0("<h5>Number of detected inconsistency between main and died_member:<strong> ",if(exists('loop_counts_main_6'))
{nrow(loop_counts_main_6)
}else {
0
},"</strong></h5>"))
```
...
```{r}
(healthyr::plot_agepyramid(table_age_pyramide))
```

#### Age Years Distibution (0 - 10)

```{r}
(healthyr::plot_age_years_distribution(table_age_pyramide %>% rename(age_years = "ind_age_years") %>% 
...
if(all(fcs_check_columns %in% names(raw.flag.fcs))) {
  ...
  (healthyr::plot_ridge_distribution(raw.flag.fcs, numeric_cols = c("fcs_cereal", "fcs_dairy", "fcs_veg", "fcs_fruit", "fcs_legumes", "fcs_sugar", "fcs_oil"),
...
(healthyr::plot_ridge_distribution(raw.flag.fcs, numeric_cols = c("rcsi_lessquality", "rcsi_borrow", "rcsi_mealsize","rcsi_mealadult", "rcsi_mealnb"),
...
if(all(fcs_check_columns %in% names(raw.flag.fcs)) & 
  ...
  (healthyr::plot_correlogram(raw.flag.fcs, numeric_cols = c("fcs_score",  "rcsi_score",  "hhs_score")))
}
```
...
(plot_age_months_distribution_iphra(raw.flag.nut, age_min = 6, age_max = 59))
```

#### Age Month Distribution per Enumerator

```{r}
(plot_age_months_distribution_iphra(raw.flag.nut, age_min = 6, age_max = 59,by_group = "enumerator"))
```
...
```{r}
(healthyr::plot_cumulative_distribution(df = raw.flag.nut, index = "muac", flags = "no"))
```
...
```{r}
(healthyr::plot_cumulative_distribution(df = raw.flag.nut, index = "muac", flags = "yes", grouping = "enumerator"))
```
...
if(!is.null(raw.water_count_loop)){
  raw.flag.wash <- raw.main %>% 
    check_WASH_flags(date_dc_date = "today", is.loop = T) 
} else {
  raw.flag.wash <- raw.main %>% 
